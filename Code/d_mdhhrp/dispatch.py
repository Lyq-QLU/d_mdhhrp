...
try:  # pragma: no cover - package/script dual use
    ...
    from .models import DMDHHRP_Environment, DynamicPatient, Solution, Route
...
def dispatch_dynamic_patients(
    ...
    for patient in ordered_waiting:
        ...
        # Try GNN-guided top-m centers + full-position feasibility search
        best = None
        weight_cost = 1.0
        weight_sat = 100.0
        for depot_id in candidate_center_ids:
            # find or create route for depot
            route = None
            for r in environment.routes:
                if r.depot.id == depot_id:
                    route = r
                    break
            if route is None:
                depot_obj = next((d for d in environment.depots if d.id == depot_id), None)
                if depot_obj is None:
                    continue
                route = Route(depot_obj)
            ...
            for pos in range(len(route.patients) + 1):
                ...
                if not eval_res.get("feasible", False):
                    continue
                score = float(eval_res["delta_cost"]) - weight_sat * float(eval_res["delta_satisfaction"]) * 1.0
                if best is None or score < best["score"]:
                    best = {"route": route, "pos": pos, "score": score}

        success = False
        if best is not None:
            target_route = best["route"]
            pos = best["pos"]
        ...
        if not success:
            # fallback to original greedy insert if configured
